...
def main():
    """Main function to analyze detailed activity data"""
    parser = argparse.ArgumentParser(description='Detailed Strava Activity Analyzer')
    parser.add_argument('--days', type=int, default=7, help='Number of days to fetch data for')
    parser.add_argument('--activity_type', type=str, default=None, help='Type of activity to analyze (e.g., Ride, Run)')
    parser.add_argument('--activity_id', type=str, default=None, help='Specific activity ID to analyze')
    parser.add_argument('--all', action='store_true', help='Process all activities in range')
    
    args = parser.parse_args()
    
    analyzer = DetailedActivityAnalyzer()
    
    if args.activity_id:
        # Process a specific activity
        print(f"Processing detailed data for activity {args.activity_id}")
        activity_data, streams_df = analyzer.process_activity_data(args.activity_id)
        
        if activity_data and streams_df is not None:
            analysis = analyzer.analyze_streams(streams_df, activity_data)
            if analysis:
                # Save analysis to JSON
                analysis_path = f'data/detailed/{args.activity_id}_analysis.json'
                with open(analysis_path, 'w') as f:
                    json.dump(analysis, f, cls=NumpyEncoder)
                print(f"Analysis saved to {analysis_path}")
            
            # Generate visualizations
            analyzer.generate_activity_visualizations(streams_df, args.activity_id, activity_data.get('name', 'Activity'))
    else:
        # Process multiple activities
        activities_df = analyzer.get_activities(days=args.days, activity_type=args.activity_type)
        ...
        if activities_df is not None and len(activities_df) > 0:
            ...
            if args.all:
                ...
                for idx, row in activities_df.iterrows():
                    ...
                    activity_data, streams_df = analyzer.process_activity_data(activity_id)
                    ...
                    if activity_data and streams_df is not None:
                        ...
                        # Generate visualizations
                        analyzer.generate_activity_visualizations(streams_df, activity_id, row['name'])
            else:
                ...
                print("\nTo analyze a specific activity, use: python src/detailed_activity.py --activity_id <ID>")
                print("To analyze all activities in range, use: python src/detailed_activity.py --all")
        else:
